...
export class OptFrontendWithTestcases extends OptFrontendSharedSessions {
  ...
  parseQueryString() {
    ...
    if (this.optTests) {
      // 添加箭头元数据参数解析
      this.params.arrowMetadataUrl = $.bbq.getState('arrowMetadataUrl');
      this.params.arrowMetadata = $.bbq.getState('arrowMetadata');
      // 如果arrowMetadata是json字符串，解析为对象
      if (this.params.arrowMetadata) {
        try {
          this.params.arrowMetadata = JSON.parse(this.params.arrowMetadata);
        } catch (e) {
          console.error('Failed to parse arrowMetadata JSON:', e);
          this.params.arrowMetadata = null;
        }
      }
      super.parseQueryString();
    }
