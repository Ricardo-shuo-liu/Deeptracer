...
export class ExecutionVisualizer {
  ...
  // 添加箭头属性
  arrowMetadata: any[]=[]; // 存储所有步骤的箭头元数据
  metadataLoaded: boolean = false; // 标记元数据是否已加载
  ...
  constructor(domRootID, dat, params) {
    ...
    this.loadArrowMetadata(params.arrowMetadataUrl || params.arrowMetadata);
  }
  //加载箭头元数据
  loadArrowMetadata(metadata: any) {
    if (typeof metadata === 'string') {
      ...
      $.ajax({
        ...
        dataType: 'json',
        success: (data)=> {
          this.arrowMetadata =  data.arrowMetadata || [];
          this.metadataLoaded = true;
          this.updateOutput(); //更新可视化
        },
        error: (jqXHR, textStatus, errorThrown)=> {
          console.error('Failed to load arrow metadata:', textStatus, errorThrown);
          this.metadataLoaded = false;
        }
      });
    } else if (metadata && typeof metadata === 'object') {
      // 如果传入的是json对象
      this.arrowMetadata = metadata.arrowMetadata || [];
      this.metadataLoaded = true;
      this.updateOutput(); //更新可视化
    }
  }  
  ...
  render() {
    ...
    if (this.params.embeddedMode) {
      ...
      base.find('#creditsPane').html('<div style="font-size: 8pt; margin-top: 5px; margin-bottom: 5px;">Rendered by <a href="http://pythontutor.com/" target="_blank" style="color: #3D58A2;">Python Tutor</a></div>');
